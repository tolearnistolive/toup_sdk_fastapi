...
class MainWidget(QWidget):
    ...
    def __init__(self):
        ...
        gboxsnap = QGroupBox("Snapshot")
        self.cmb_snap_res = QComboBox()
        self.cmb_snap_res.setEnabled(False)
        self.lbl_snap_info = QLabel("Snap: —")
        vlytsnap = QVBoxLayout()
        vlytsnap.addWidget(QLabel("Capture resolution:"))
        vlytsnap.addWidget(self.cmb_snap_res)
        vlytsnap.addWidget(self.lbl_snap_info)
        gboxsnap.setLayout(vlytsnap)
    ...
    def closeCamera(self):
        ...
        self.cmb_res.clear()
        self.cmb_snap_res.setEnabled(False)
        self.cmb_snap_res.clear()
        self.lbl_snap_info.setText("Snap: —")
    ...
    def startCamera(self):
        ...
        try:
            ...
            self.cmb_snap_res.setEnabled(self.cur.model.still > 0)
    ...
    def openCamera(self):
        ...
        self.res = self.hcam.get_eSize()  # current preview index

        # Fill preview resolutions
        with QSignalBlocker(self.cmb_res):
            self.cmb_res.clear()
            for i in range(self.cur.model.preview):
                w = self.cur.model.res[i].width
                h = self.cur.model.res[i].height
                self.cmb_res.addItem(f"Preview: {w} × {h}", i)
            self.cmb_res.setCurrentIndex(self.res)

        # Fill still / snap resolutions
        with QSignalBlocker(self.cmb_snap_res):
            self.cmb_snap_res.clear()
            still_count = self.cur.model.still

            if still_count == 0:
                # fallback: same as current preview
                self.cmb_snap_res.addItem("Same as preview", self.res)
                self.lbl_snap_info.setText("Snap: same as live view")
            else:
                for i in range(still_count):
                    w = self.cur.model.res[i].width
                    h = self.cur.model.res[i].height
                    self.cmb_snap_res.addItem(f"Snap: {w} × {h}", i)
                # default to highest resolution
                self.cmb_snap_res.setCurrentIndex(still_count - 1)
                w_max = self.cur.model.res[still_count-1].width
                h_max = self.cur.model.res[still_count-1].height
                self.lbl_snap_info.setText(f"Snap: {w_max} × {h_max} (best)")

            self.cmb_snap_res.setEnabled(still_count > 0)
    ...
    def onBtnSnap(self):
        ...
        if still_count == 0:
            # fallback: save current live frame
            if self.pData:
                image = QImage(self.pData, self.imgWidth, self.imgHeight, QImage.Format_RGB888)
                self.count += 1
                filename = f"snapshot_{self.count}_preview_{self.imgWidth}x{self.imgHeight}.jpg"
                image.save(filename)
                print(f"Saved preview-quality snapshot: {filename}")
        else:
            # use selected still resolution
            snap_index = self.cmb_snap_res.currentData()
            if snap_index is None:
                snap_index = still_count - 1  # highest

            print(f"Requesting still image at index {snap_index}")
            try:
                self.hcam.Snap(snap_index)
                # the image will arrive via TOUPCAM_EVENT_STILLIMAGE
            except toupcam.HRESULTException as e:
                QMessageBox.warning(self, "Snap failed", f"Error: {e}")
    ...
    def onevtCallback(self, nEvent):
        ...
        if self.hcam:
            ...
            elif toupcam.TOUPCAM_EVENT_DISCONNECTED == nEvent:
                self.closeCamera()
    ...
    def handleStillImageEvent(self):
        ...
        try:
            ...
            self.count += 1
            filename = f"snapshot_{self.count}_full_{info.width}x{info.height}.jpg"
            image.save(filename)
            print(f"Saved high-quality snapshot: {filename}")

            # Optional: show a small message or thumbnail
            QMessageBox.information(self, "Snapshot saved", f"Saved {info.width}×{info.height} image")

        except toupcam.HRESULTException as e:
            print("Still image pull failed:", e)
